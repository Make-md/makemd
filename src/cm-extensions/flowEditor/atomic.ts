import { Range, EditorState, Transaction, EditorSelection, TransactionSpec, StateField, RangeSetBuilder, Annotation } from '@codemirror/state';
import {
	keymap, EditorView,
...
export const atomicSelect =  EditorState.transactionFilter.of((tr:Transaction) => {
    ...
    if (tr.annotation(arrowKeyAnnotation) && flowID) {
        ...
        const lineRange = tr.state.field(selectiveLinesFacet, false)
            const posRange = lineRange && lineRange[0] != undefined ? lineRangeToPosRange(tr.startState, lineRange) : { from: 0, to: tr.startState.doc.length};
