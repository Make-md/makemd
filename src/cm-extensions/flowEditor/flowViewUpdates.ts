...
import { flowIDStateField, flowTypeStateField, portalTypeAnnotation } from 'cm-extensions/markSans/callout';
import { MarkdownView } from 'obsidian';
import { loadFlowEditorsForLeaf } from 'utils/flowEditor';
import { cacheFlowEditorHeight, flowEditorInfo, toggleFlowEditor } from './flowEditor';
    ...
    if (v.heightChanged ) {
      app.workspace.iterateRootLeaves((leaf) => {
        ...
        if (cm && v.view.dom == cm.dom && cm.state.field(flowTypeStateField, false)) {
          if(leaf.containerEl.parentElement?.hasClass('workspace-tab-container')) {
