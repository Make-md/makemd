...
import {StateField, RangeSetBuilder, Annotation, EditorState, Transaction, TransactionSpec, EditorSelection, SelectionRange} from '@codemirror/state'
...
export const toggleFlowEditor = Annotation.define<[id: string, state: number]>();
export const cacheFlowEditorHeight = Annotation.define<[id: string, height: number]>();

export const preloadFlowEditor =  EditorState.transactionFilter.of((tr:Transaction) => {
  
  let newTrans = [] as TransactionSpec[];
	const value = tr.state.field(flowEditorInfo, false);
  if (value && !tr.annotation(toggleFlowEditor)) {
      newTrans.push(...value.filter(f => f.expandedState == 1).map(f => {
...
export const internalLinkHover = hoverTooltip((view, pos, side) => {
    ...
    iterateTreeInSelection({from: lineFrom, to: lineTo}, view.state, {
              ...
              if (info) {
                hovObject = {
                    ...
                    create(view: EditorView) {
                      ...
                      openHoverDiv.setAttribute('aria-label', info.expandedState == 0 ? t.buttons.openFlow : t.buttons.hideFlow)
      ...
      openHoverDiv.insertAdjacentText('beforeend', info.expandedState == 0 ? t.buttons.openFlow : t.buttons.hideFlow)
                      return {dom}
                    }
                  }
                  return false;
  ...
  const findFullInternalLink = (posA: number, posB:number, state: EditorState) : SelectionRange | null => {
            ...
            if (name.contains('hmd-internal-link')) {
              ...
              const fullRange = findFullInternalLink(from, to, tr.state);
                ...
                const info = {
                  ...
                  to: fullRange.to,
                  embed: embedType,
                  height: existingInfo ? tr.annotation(cacheFlowEditorHeight)?.[0] == id && tr.annotation(cacheFlowEditorHeight)?.[1] != 0 ? tr.annotation(cacheFlowEditorHeight)?.[1] : existingInfo.height : -1,
                  expandedState: existingInfo ? tr.annotation(toggleFlowEditor)?.[0] == id ? reverseExpandedState(existingInfo.expandedState) :
                  existingInfo.expandedState : embedType >= 1 ? 1 : 0,
              }
              newValues.push(info);
            }
          }
...
class FlowEditorWidget extends WidgetType {
  ...
  class FlowEditorSelector extends WidgetType {
    ...
    toDOM(view: EditorView) {
        ...
        reactEl.render(<FlowEditorHover toggle={true} toggleState={true} toggleFlow={() => {
          view.dispatch({
        ...
        app.workspace.iterateLeaves((leaf) => {
            ...
            if (cm && view.dom == cm.dom) {
              openFileFlowEditor(this.info.link, (leaf.view as MarkdownView).file?.path);
              // return true;
            }
  ...
  export const flowEditorSelector = (info: FlowEditorInfo) => Decoration.replace({
    widget: new FlowEditorSelector(info),
    inclusive: true,
    block: false,
  })

  export const flowEditorDecoration = (info: FlowEditorInfo) => Decoration.replace({
    widget: new FlowEditorWidget(info),
    inclusive: true,
    block: false,
  })

  export const flowEditorWidgetDecoration = (info: FlowEditorInfo) => Decoration.widget({
    widget: new FlowEditorWidget(info),
    block: true,
  })
