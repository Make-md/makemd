...
export const loadStylerIntoContainer = (el: HTMLElement) => {
  ...
  root.render(<InlineMenuComponent mobile={true} activeMarks={[]}></InlineMenuComponent>)
}
...
export const InlineMenuComponent: React.FC <{cm?: EditorView, activeMarks: string[], mobile: boolean}> = (props) => {
  ...
  const toggleMarkAction = (e: React.MouseEvent, s: InlineStyle) => {
    ...
    const makeMode = () => <>
    <div aria-label={!platformIsMobile() ? t.commands.makeMenu : undefined}  onMouseDown={(e) => {
    makeMenu(e);
  }} className='mk-mark' dangerouslySetInnerHTML={{__html: markIconSet['mk-make-slash']}}>
  </div>
  <div aria-label={!platformIsMobile() ? t.commands.selectStyle : undefined}  onMouseDown={() => {
    setMode(1);
  }} className='mk-mark' dangerouslySetInnerHTML={{__html: markIconSet['mk-make-style']}}>
  </div>
  <div aria-label={!platformIsMobile() ?  t.commands.image : undefined}  onMouseDown={() => {
  ...
  <div aria-label={!platformIsMobile() ?  t.commands.toggleKeyboard : undefined}  onMouseDown={() => {
    const view = getActiveMarkdownView();
...
const colorsMode = () => <>
<div className='mk-mark' onMouseDown={() => {
  setColorMode(null);
  setMode(1);
  }} dangerouslySetInnerHTML={{__html: uiIconSet['mk-ui-close']}}>
</div>
{ colors.map((c, i) => <div key={i} onMouseDown={() => {
  setMode(1);
  setColorMode(null)
  const cm = props.cm ?? getActiveCM();
  if (!cm)
...
}} className='mk-color' style={{background: c}}></div>) }</>

    const marksMode = () => <> 
    {
    props.mobile ? <div className='mk-mark' onMouseDown={() => {
  setMode(0);
  }} dangerouslySetInnerHTML={{__html: uiIconSet['mk-ui-close']}}>
    </div> : <></>}{
        resolveStyles().map((s, i) => 
        {
        return <Mark i={i} style={s} active={props.activeMarks.find(f => f == s.mark) ? true : false} toggleMarkAction={toggleMarkAction}></Mark>
        }) } 
        { window.make.settings.inlineStylerColors ?
        <><div className='mk-divider'></div> 
  <div aria-label={!platformIsMobile() ?  t.styles.textColor : undefined} onMouseDown={() => 
    {
      setMode(2);
  ...
  <div aria-label={!platformIsMobile() ? t.styles.highlight : undefined}  onMouseDown={() => {
    setMode(2);
  ...
  : <></>}
    ...
    return <div className={classNames('mk-style-menu', props.mobile ? '' : 'menu')} onMouseDown={(e) => e.preventDefault()}>
      {
        mode == 0 && props.mobile ? 
        makeMode() : 
        mode == 2 ?
