...
import { SpaceTable, SpaceTableSchema, SpaceProperty, SpaceTables } from "shared/types/mdb";
...
interface SpaceManagerContextType {
  ...
  saveProperties(path: string, properties: Record<string, any>): Promise<boolean>;
...
export const SpaceManagerProvider: React.FC<SpaceManagerProviderProps> = ({
  ...
  const convertMKitPath = useCallback((path: string): string => {
    if (!isMKitPath(path)) {
      return path;
    }
  ...
  const readTable = useCallback(async (path: string, schema: string): Promise<SpaceTable | null> => {

    if (mkitContext?.isPreviewMode && isMKitPath(path)) {
      // Handle MKit preview mode
      const lookupPath = convertMKitPath(path);
      
      const mkitSpaceData = mkitContext.getSpaceByFullPath(lookupPath) || 
  ...
  const saveTable = useCallback(async (path: string, table: SpaceTable, force?: boolean): Promise<boolean> => {
  ...
  const readFrame = useCallback(async (path: string, schema: string): Promise<MDBFrame | null> => {
  ...
  const saveFrame = useCallback(async (path: string, frame: MDBFrame): Promise<void> => {
  ...
  // Schema operations
  const tablesForSpace = useCallback(async (path: string): Promise<SpaceTableSchema[]> => {
  ...
  const framesForSpace = useCallback(async (path: string): Promise<SpaceTableSchema[]> => {
    ...
    if (mkitContext?.isPreviewMode && isMKitPath(path)) {
      ...
      if (mkitSpaceData?.frameSchemas) {
        return mkitSpaceData.frameSchemas.map(fs => fs as any as SpaceTableSchema);
      } else {
      }
    }
  ...
  // Path operations
  const resolvePath = useCallback((path: string, source?: string): string => {
  ...
  const uriByString = useCallback((uri: string, source?: string): URI => {
  ...
  const createSpace = useCallback((name: string, parentPath: string, definition: SpaceDefinition): void => {
  ...
  const spaceInfoForPath = useCallback((path: string): SpaceInfo => {
    if (superstate?.spaceManager) {
  ...
  const contextForSpace = useCallback(async (path: string): Promise<SpaceTable> => {
  ...
  const addSpaceProperty = useCallback(async (path: string, property: SpaceProperty): Promise<boolean> => {
  ...
  const saveProperties = useCallback(async (path: string, properties: Record<string, any>): Promise<boolean> => {
  ...
  const renameProperty = useCallback((path: string, property: string, newProperty: string): void => {
  ...
  const createItemAtPath = useCallback(async (parent: string, type: string, name: string, content?: any): Promise<string> => {
    
    if (superstate?.spaceManager) {
      return await superstate.spaceManager.createItemAtPath(parent, type, name, content);
    }
  ...
  const writeToPath = useCallback(async (path: string, content: any, binary?: boolean): Promise<void> => {
  ...
  const parentPathForPath = useCallback((path: string): string => {
    if (superstate?.spaceManager) {
  ...
  const childrenForSpace = useCallback((path: string): string[] => {
    if (superstate?.spaceManager) {
  ...
  const spaceInitiated = useCallback(async (path: string): Promise<boolean> => {
    if (superstate?.spaceManager) {
  ...
  const contextInitiated = useCallback(async (path: string): Promise<boolean> => {
    if (superstate?.spaceManager) {
  ...
  const readAllTables = useCallback(async (path: string): Promise<SpaceTables> => {
    if (mkitContext?.isPreviewMode && isMKitPath(path)) {
      const convertedPath = convertMKitPath(path);
      const spaceData = mkitContext.getSpaceByFullPath(convertedPath) || 
  ...
  const readAllFrames = useCallback(async (path: string): Promise<MDBFrames> => {
    if (mkitContext?.isPreviewMode && isMKitPath(path)) {
      const convertedPath = convertMKitPath(path);
      const spaceData = mkitContext.getSpaceByFullPath(convertedPath) || 
                       mkitContext.getSpaceByRelativePath(convertedPath);
  ...
  const saveSpace = useCallback((path: string, definition: (def: SpaceDefinition) => SpaceDefinition, properties?: Record<string, any>): void => {
    if (superstate?.spaceManager) {
      superstate.spaceManager.saveSpace(path, definition, properties);
    }
  }, [superstate]);

  const renameSpace = useCallback(async (path: string, newPath: string): Promise<string> => {
    if (superstate?.spaceManager) {
  ...
  const spaceDefForSpace = useCallback(async (path: string): Promise<SpaceDefinition> => {
    if (superstate?.spaceManager) {
  ...
  const allPaths = useCallback((type?: string[]): string[] => {
    if (superstate?.spaceManager) {
  ...
  const renamePath = useCallback(async (oldPath: string, newPath: string): Promise<string> => {
    if (superstate?.spaceManager) {
  ...
  const copyPath = useCallback(async (source: string, destination: string, newName?: string): Promise<string> => {
    if (superstate?.spaceManager) {
      return await superstate.spaceManager.copyPath(source, destination, newName);
    }
    return '';
  }, [superstate]);

  const getPathInfo = useCallback(async (path: string): Promise<Record<string, any>> => {
    if (superstate?.spaceManager) {
  ...
  const readPathCache = useCallback(async (path: string): Promise<PathCache> => {
    if (superstate?.spaceManager) {
  ...
  const getPathState = useCallback((path: string): PathState | null => {
    if (mkitContext?.isPreviewMode && isMKitPath(path)) {
  ...
  const childrenForPath = useCallback(async (path: string, type?: string): Promise<string[]> => {
    if (superstate?.spaceManager) {
  ...
  const saveFrameSchema = useCallback(async (path: string, schemaId: string, saveSchema: (prev: SpaceTableSchema) => SpaceTableSchema): Promise<void> => {
    
    if (superstate?.spaceManager) {
      await superstate.spaceManager.saveFrameSchema(path, schemaId, saveSchema);
    }
  }, [superstate]);

  const deleteFrame = useCallback(async (path: string, name: string): Promise<void> => {
  ...
  const contextValue = useMemo<SpaceManagerContextType>(() => ({
    ...
    readTable, saveTable, readFrame, saveFrame,
    tablesForSpace, framesForSpace,
    resolvePath, uriByString, pathExists,
    createSpace, deleteSpace, spaceInfoForPath, contextForSpace,
    addSpaceProperty, saveProperties, deleteProperty, renameProperty,
    createItemAtPath, deletePath, readPath, writeToPath, parentPathForPath,
    allSpaces, childrenForSpace, spaceInitiated, contextInitiated,
    readAllTables, readAllFrames, saveSpace, renameSpace, spaceDefForSpace,
    allPaths, renamePath, copyPath, getPathInfo, readPathCache, getPathState, childrenForPath,
    saveFrameSchema, deleteFrame,
    mkitContext?.isPreviewMode, convertMKitPath, isMKitPath,
 ...
 */
export const MKitSpaceManagerProvider: React.FC<MKitSpaceManagerProviderProps> = ({
  mkitContext,
  children,
}) => {
  ...
  const saveSpace = useCallback((path: string, definition: (def: SpaceDefinition) => SpaceDefinition, properties?: Record<string, any>): void => {}, []);
  const renameSpace = useCallback(async (path: string, newPath: string): Promise<string> => { return ''; }, []);
  const spaceDefForSpace = useCallback(async (path: string): Promise<SpaceDefinition> => { return null; }, []);
  const allPaths = useCallback((type?: string[]): string[] => { return mkitContext?.getAllRelativePaths() || []; }, [mkitContext]);
  const renamePath = useCallback(async (oldPath: string, newPath: string): Promise<string> => { return ''; }, []);
  const copyPath = useCallback(async (source: string, destination: string, newName?: string): Promise<string> => { return ''; }, []);
  const getPathInfo = useCallback(async (path: string): Promise<Record<string, any>> => { return {}; }, []);
  const readPathCache = useCallback(async (path: string): Promise<PathCache> => { return null; }, []);
  const getPathState = useCallback((path: string): PathState | null => {
    const convertedPath = convertMKitPath(path);
    const spaceData = mkitContext?.getSpaceByFullPath(convertedPath) || 
                     mkitContext?.getSpaceByRelativePath(convertedPath);
  ...
  const childrenForPath = useCallback(async (path: string, type?: string): Promise<string[]> => { return []; }, []);
  const saveFrameSchema = useCallback(async (path: string, schemaId: string, saveSchema: (prev: SpaceTableSchema) => SpaceTableSchema): Promise<void> => {
    // Read-only in MKit
  }, []);
  const deleteFrame = useCallback(async (path: string, name: string): Promise<void> => {
