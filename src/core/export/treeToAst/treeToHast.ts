...
const tableViewForContext = async (superstate: Superstate, predicate: Predicate, cols: SpaceProperty[], data:  Record<string, DBRows>, path: string) : Promise<Node> => {
...
const contextNodeToHast = async (superstate: Superstate, node: FrameNode, instance: FrameRunInstance, og: string) => {
  ...
  const { type, instances, predicate, cols, data, path, source } = contextNode;
    if (type == 'table') {
      return [await tableViewForContext(superstate, predicate, cols, data, path)];
    }
...
export const treeNodeToHast =  async (superstate: Superstate, treeNode: FrameTreeNode, instance: FrameRunInstance, content: Node[], path: string) :Promise<Node[]> => {
  ...
  const treeNodeType = treeNode.node.type;
    ...
    if (treeNodeType === 'space') {
        inner = await contextNodeToHast(superstate, node, instance, path);
    }
  ...
  if (treeNodeType == 'flow') {
    ...
    const exec = await buildExecutable(tree);
        ...
        const hast = await treeNodeToHast(superstate, tree, _instance, [], parent);
    inner = hast ?? [];
}
...
if (treeNodeType == 'slides') {
  ...
  if (treeNode.children?.length > 0) {
    ...
    for (let i = 0; i < treeNode.children.length; i++) {
      const node = await treeNodeToHast(superstate, treeNode.children[i], instance, content, path)
      if (node)
        inner.push(...node);
    }
  }
...
export const instanceToHast = async (superstate: Superstate, instance: FrameRunInstance, content: Node[], path: string, rootType?: string) => {
  ...
  if (rootType == 'contextGroup' || rootType == 'contextView') {
    ...
    return treeNodeToHast(superstate, instance.exec, instance, content, path);
}

export const treeToHast = async (superstate: Superstate, context: FrameContexts, path: string, schema: string) => {
  const instance = await getFrameInstanceFromPath(superstate, path, schema, {}, context);
