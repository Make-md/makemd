...
const row: React.FC<ListChildComponentProps> = memo(({data, index, style}) => 
  {
    ...
    return <SortableTreeItem 
            ...
            indicator={projected?.overId == f.id ? f.parentId == null && projected.parentId == null ?  { state: IndicatorState.Top, depth:projected.depth } : { state: IndicatorState.Bottom, depth:projected.depth } : null}
            plugin={plugin}
            style={style}
            collapsed={f.parentId == null ? f.id == '/' ? plugin.settings.vaultCollapsed : sections[f.index].collapsed : !openFolders.find((i: string) => i == f.id)}
            onCollapse={
              handleCollapse
            }
            ></SortableTreeItem>
...
export const FileExplorerComponent = (props: FileExplorerComponentProps) => {
  ...
  const rowHeight = (index: number) =>  isMobile ? flattenedItems[index].parentId == null ? 60 : 40 : flattenedItems[index].parentId == null ? 44 : 29;
    ...
    return <div className='mk-file-tree'>
      <DndKitContext
    sensors={sensors}
    ...
    onDragStart={handleDragStart}
      ...
      >
        <SortableContext items={sortedIds} strategy={verticalListSortingStrategy}>
          <AutoSizer>
            {({ height, width}) =>
          <List ref={listRef} height={height} itemSize={rowHeight} itemCount={itemData.flattenedItems.length} itemData={itemData} width={width} overscanCount={plugin.settings.spacesPerformance ? 0 : 20}>
            {row}
            </List>
            }
            </AutoSizer>
