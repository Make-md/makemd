...
export const FileRow = (props: FileRowProps) => {
    ...
    return <tr className='mk-file-row'>
        <td className='mk-column-icon' dangerouslySetInnerHTML={item.icon ?? item.type == 'folder' ? 
               {__html: uiIconSet['mk-ui-folder']} : {__html: uiIconSet['mk-ui-file']}}>
        { item.icon && unifiedToNative(item.icon[1]) }
        </td>
        <td className='mk-column-file' onClick={(e) => 
        //@ts-ignore
    ...
    </td><td className='mk-file-date'>
        {item.created && dayjs(item.created).fromNow()}</td></tr>
}