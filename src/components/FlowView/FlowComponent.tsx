...
export const FolderComponent = (props: FolderComponentProps) => {
    ...
    return <div className='mk-folder-scroller'>
        <div className='mk-folder-sizer'>
            <div className='mk-folder-header'><div className='inline-title'>
                {props.folder.name}
            </div>
            
            </div>
            {filteredNotes.length > 0 ?
            <div>{filteredNotes.map((f, i) => <FlowRow key={i} item={f} plugin={props.plugin}></FlowRow>)}</div>
            :
