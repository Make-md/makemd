...
const tooltipPlugin = ViewPlugin.fromClass(
  class {
    ...
    destroy() {
      ...
      for (const { dom } of this.manager.tooltipViews) dom.remove();
      this.intersectionObserver?.disconnect();
...
class HoverPlugin {
  ...
  startHover() {
    clearTimeout(this.restartTimeout);
    const { lastMove } = this;
    const pos = this.view.contentDOM.contains(lastMove.target)
  ...
  mousemove(event: MouseEvent) {
    ...
    if ((tooltip && !isInTooltip(this.lastMove.target)) || this.pending) {
      const { pos } = tooltip || this.pending!,
        end = tooltip?.end ?? pos;
      if (
        pos == end
  ...
  const hoverState = StateField.define<Tooltip | null>({
    ...
    update(value, tr) {
      ...
      if (value && tr.docChanged) {
        ...
        const copy: Tooltip = Object.assign(Object.create(null), value);
        copy.pos = newPos;
