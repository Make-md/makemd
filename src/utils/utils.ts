...
export function buildTree(flattenedItems: FlattenedTreeNode[]): TFolder {
  ...
  for (const item of items) {
    ...
    const _parent =
      items.find((f) => f.id == parentId) ?? items.find((f) => f.id == "/");
    if (_parent) {
      if (item.isFolder) {
...
export const nodeIsAncestorOfTarget = (
  node: FlattenedTreeNode,
  target: FlattenedTreeNode
) => {
    let isAncestor = false;
    let condition = true;
    let _target : TFolder = app.vault.getAbstractFileByPath(target.path) as TFolder;
    while (condition) {
...
function getMaxDepth({ previousItem }: { previousItem: FlattenedTreeNode }) {
  if (previousItem) {
    if (previousItem.isFolder) return previousItem.depth + 1;
...
export function getProjection(
  ...
  function getParentId() {
    ...
    if (
      ...
      (depth > previousItem.depth && !previousItem.isFolder)
    ) {
...
export const flattenTrees = (
  ...
  return items
    .filter((f) => f)
    .reduce<FlattenedTreeNode[]>((acc, item, index) => {
...
export const flattenTree = (
  ...
  return [
    ...
    ...(!collapsed
      ? flattenTrees(folder.children, path, sectionIndex, path, 1)
      : []),
  ];
};
...
export const sortFolderTree = async (
  folderTree: TFolder,
  plugin: MakeMDPlugin
): Promise<TFolder> => {
  ...
  const sortedFolderTree = sortFolderTreeUsingStringTree(
    folderTree,
    newStringTree
  );
...
export const renamePathInStringTree = (
  oldPath: string,
  newFile: TAbstractFile,
  plugin: MakeMDPlugin
) => {
  const stringTree = plugin.settings.folderRank;
  ...
  const recursive = (
    ...
    return {
      ...tree,
      children: tree.children.map((f) =>
        recursive(f, path + "/" + f.node, oldS, newS)
      ),
    };
  };
  plugin.settings.fileIcons = plugin.settings.fileIcons.map((f) =>
    f[0] == oldPath ? [newPath, f[1]] : f
  );
  plugin.settings.spaces = plugin.settings.spaces.map((f) => {
    return {
      ...f,
      children: f.children.map((g) => (g == oldPath ? newPath : g)),
    };
  });

  plugin.settings.folderRank = recursive(
    stringTree,
    "",
    "/" + oldPath,
    newName
...
export const folderTreeToStringTree = (tree: TFolder): StringTree => {
  const recursive = (subtree: TAbstractFile): StringTree => {
    if ((subtree as any).children) {
      return {
        node: subtree.name,
        children: (subtree as any).children.map((f: TAbstractFile) =>
          recursive(f)
        ),
        isFolder: true,
      };
...
const reorderStringTree = (
  savedTrees: StringTree[],
  rawTrees: StringTree[]
): StringTree[] => {
  //find missing trees in live not in cache and append
  const missingTrees = rawTrees.filter(
    (f) => !savedTrees.find((g) => f.node == g.node)
  );
  const allTrees = [...savedTrees, ...missingTrees];
  //remove trees that are in cache but not in live
  const filteredTrees = allTrees.filter((f) =>
    rawTrees.find((g) => f.node == g.node)
  );
  return filteredTrees;
};

export const mergeStringTree = (
  savedTree: StringTree,
  rawTree: StringTree
): StringTree => {
  ...
  const recursive = (
    subtree: StringTree,
    treePaths: StringTreePath[],
    currPath: string
  ): StringTree => {
    const existingTree: StringTreePath | undefined = treePaths.find(
      (f) => currPath == f.path
    );
    ...
    if (existingTree) {
      return {
        ...subtree,
        children: reorderStringTree(
          existingTree.children,
          subtree.children
        ).map((t) => recursive(t, treePaths, currPath + "/" + t.node)),
      };
    } else {
      return {
        ...subtree,
        children: subtree.children.map((t) =>
          recursive(t, treePaths, currPath + "/" + t.node)
        ),
      };
    }
  };
...
export const sortFolderTreeUsingStringTree = (
  folderTree: TFolder,
    ...
    if (file instanceof TFolder) {
      return {
        ...
        children: file.children
          .map((f) => {
            const currStringTree = strings.children.find(
              (g) => g.node == f.name
            );

            if (currStringTree) return recursiveSort(f, currStringTree);
            return f;
          })
          .sort(
            (a, b) =>
              strings.children.findIndex((x) => x.node == a.name) -
              strings.children.findIndex((x) => x.node == b.name)
          ),
      };
    } else {
      return file;
...
export const openFile = async (
  ...
  if (file.isFolder) {
    ...
    let leaf = app.workspace.getLeaf(newLeaf);
    app.workspace.setActiveLeaf(leaf, { focus: true });
    await leaf.openFile(app.vault.getAbstractFileByPath(file.path) as TFile, {
      eState: { focus: true },
    });
  }
};
